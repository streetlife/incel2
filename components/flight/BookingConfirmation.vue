  ...
  <div class="space-y-6">
    <div class="bg-gradient-to-br from-green-500 to-emerald-600 rounded-2xl px-6 py-8 text-center shadow-xl shadow-green-200">
      ...
      <div class="inline-block bg-white/20 rounded-xl px-5 py-2">
        ...
        <p class="text-2xl font-bold text-white font-mono tracking-widest">{{ bookingReference }}</p>
      </div>
    </div>
    ...
    <div class="flex items-start gap-3 bg-blue-50 border border-blue-200 rounded-2xl px-4 py-4">
      ...
      <div>
        ...
        <p class="text-sm text-blue-600 mt-0.5">Invoice and e-ticket sent to <span class="font-mono font-bold">{{ contactEmail }}</span></p>
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
      <div class="px-5 py-4 bg-slate-800 flex items-center justify-between">
        ...
        <span class="text-slate-400 text-xs font-mono">{{ invoiceNumber }}</span>
      </div>
      ...
      <div class="px-5 py-4 border-b border-slate-100">
        ...
        <div class="flex items-center justify-between">
          <div class="text-center">
            <p class="text-3xl font-bold text-slate-900 font-mono">{{ fmtTime(offer?.itineraries[0].segments[0].departure.at ?? '') }}</p>
            <p class="text-sm font-bold text-slate-600 mt-1">{{ offer?.itineraries[0].segments[0].departure.iataCode }}</p>
            <p class="text-xs text-slate-400">{{ fmtDate(offer?.itineraries[0].segments[0].departure.at ?? '') }}</p>
          </div>
          <div class="flex flex-col items-center flex-1 px-4">
            <p class="text-xs text-slate-400 mb-1">{{ parseDur(offer?.itineraries[0].duration ?? 'PT0H') }}</p>
            ...
            <p class="text-xs text-green-600 font-medium mt-1">
              {{ offer?.itineraries[0].segments.length === 1 ? 'Non-stop' : `${offer?.itineraries[0].segments.length - 1} stop` }}
            </p>
          </div>
          <div class="text-center">
            <p class="text-3xl font-bold text-slate-900 font-mono">{{ fmtTime(offer?.itineraries[0].segments.at(-1)?.arrival.at ?? '') }}</p>
            <p class="text-sm font-bold text-slate-600 mt-1">{{ offer?.itineraries[0].segments.at(-1)?.arrival.iataCode }}</p>
            <p class="text-xs text-slate-400">{{ fmtDate(offer?.itineraries[0].segments.at(-1)?.arrival.at ?? '') }}</p>
          </div>
        </div>
      ...
      <div class="px-5 py-4">
        ...
        <div class="space-y-3">
          <div
            v-for="(p, i) in passengers" :key="i"
            class="flex items-center justify-between py-2 border-b border-slate-100 last:border-0"
          >
            <div>
              <p class="text-sm font-semibold text-slate-800">{{ p.title }} {{ p.firstname }} {{ p.surname }}</p>
              <p class="text-xs text-slate-400">{{ p.passport_nationality }}</p>
            </div>
            <div class="text-right">
              <p class="text-[10px] text-slate-400 mb-0.5">Ticket No.</p>
              <p class="font-mono text-xs font-bold text-primary">{{ ticketNumbers[i] }}</p>
            </div>
          </div>
        </div>
      </div>
