...
function resolveColor(code: string): string {
  return FALLBACK_COLORS[code] ?? GENERATED_COLORS[(code.codePointAt(0) ?? 0) % GENERATED_COLORS.length]
}
function resolveName(code: string): string {
  return props.airlines?.[code]?.name ?? code
}
function resolveLogo(code: string): string | null {
  const raw = props.airlines?.[code]?.logo
  if (!raw) return null
  const lastHttp = raw.lastIndexOf('http', raw.length - 5)
  return lastHttp > 0 ? raw.slice(lastHttp) : raw
}
...
const carrierName = computed(() => resolveName(carrier.value))
const carrierLogo = computed(() => resolveLogo(carrier.value))
const color = computed(() => resolveColor(carrier.value))
const showLogo = computed(() => !!carrierLogo.value && !logoErrors.value[carrier.value])
const fare = computed(() => props.flight.travelerPricings[0].fareDetailsBySegment[0])
const bags = computed(() => props.flight.price.additionalServices?.find(s => s.type === 'CHECKED_BAGS'))
const stops = computed(() => segs.value.length - 1)
const via = computed(() => segs.value.slice(0, -1).map(s => s.arrival.iataCode).join(' · '))
  ...
  >
    <div class="md:hidden p-4 flex flex-col gap-3">
      <div class="flex items-center justify-between">
        <div class="flex items-center gap-3">
          <div class="w-10 h-10 rounded-xl overflow-hidden shrink-0 flex items-center justify-center"
            :style="showLogo ? {} : { background: color }">
            <img v-if="showLogo"
              :src="carrierLogo!"
              :alt="carrier"
              class="w-full h-full object-contain p-1 bg-white"
              @error="onLogoError(carrier)"
            />
            <span v-else class="text-white text-xs font-bold tracking-wide">{{ carrier }}</span>
          </div>
          <div>
      ...
      <div class="flex gap-1.5 overflow-x-auto pb-0.5 scrollbar-hide">
        ...
        <span class="inline-flex shrink-0 items-center gap-1 text-xs font-medium px-2 py-0.5 rounded-md whitespace-nowrap border"
          :class="(fare.includedCheckedBags.quantity ?? 0) === 0 ? 'bg-orange-50 border-orange-200 text-orange-700' : 'bg-slate-50 border-slate-200 text-slate-500'">
          {{ (fare.includedCheckedBags.quantity ?? 0) > 0 ? `${fare.includedCheckedBags.quantity} checked` : 'No checked bag' }}
        </span>
    ...
    <div class="hidden md:grid md:items-center md:gap-5 md:p-5" style="grid-template-columns: 148px 1fr auto auto;">
      <div class="flex flex-col items-center gap-2 text-center">
        <div class="w-12 h-12 rounded-xl overflow-hidden flex items-center justify-center shrink-0"
          :style="showLogo ? { border: '1px solid #e2e8f0' } : { background: color }">
          <img v-if="showLogo"
            :src="carrierLogo!"
            :alt="carrier"
      ...
      <div class="flex flex-col gap-1.5 items-start">
        ...
        <span class="inline-flex items-center gap-1 text-xs font-medium text-slate-500 bg-slate-50 border border-slate-200 px-2 py-0.5 rounded-md whitespace-nowrap">
          ...
          {{ fare.includedCabinBags.quantity ?? fare.includedCabinBags.weight + 'kg' }} cabin
        </span>
        <span class="inline-flex items-center gap-1 text-xs font-medium px-2 py-0.5 rounded-md whitespace-nowrap border"
          :class="(fare.includedCheckedBags.quantity ?? 0) === 0 ? 'bg-orange-50 border-orange-200 text-orange-700' : 'bg-slate-50 border-slate-200 text-slate-500'">
          {{ (fare.includedCheckedBags.quantity ?? 0) > 0 ? `${fare.includedCheckedBags.quantity} checked` : 'No checked bag' }}
        </span>
        ...
        <div v-for="(seg, idx) in segs" :key="idx" class="mb-4 last:mb-0">
          ...
          <div class="flex items-center gap-2 mb-2.5">
            <div class="w-5 h-5 rounded overflow-hidden shrink-0 flex items-center justify-center"
              ...
              <img v-if="resolveLogo(seg.carrierCode) && !logoErrors[seg.carrierCode]"
                :src="resolveLogo(seg.carrierCode)!"
                :alt="seg.carrierCode"
                class="w-full h-full object-contain bg-white p-0.5"
                @error="onLogoError(seg.carrierCode)"
              />
              <span v-else class="text-white text-[8px] font-bold">{{ seg.carrierCode }}</span>
            </div>
            <span class="text-xs font-semibold text-slate-500">
              {{ resolveName(seg.carrierCode) }} · Flight {{ seg.carrierCode }}{{ seg.number }}
            </span>
          </div>
