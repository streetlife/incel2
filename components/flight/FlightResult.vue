...
<template>
  <div class="font-sans text-slate-800">
    ...
    >
      <div v-if="showMobileFilters" class="md:hidden bg-white rounded-2xl border border-slate-200 shadow-sm p-4 mb-3">
        <div class="flex items-center justify-between mb-3">
          <span class="text-sm font-bold text-slate-900">Filters</span>
        ...
        <div class="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <label v-for="s in [0, 1, 2]" :key="s" class="flex items-center gap-2 py-0.5 text-sm text-slate-600 cursor-pointer">
              <input type="checkbox" class="w-3.5 h-3.5 accent-primary cursor-pointer" :checked="filters.stops.includes(s)" @change="toggleFilter(filters.stops, s)" />
          ...
          <div>
            ...
            <label v-for="c in allCabins" :key="c" class="flex items-center gap-2 py-0.5 text-sm text-slate-600 cursor-pointer">
              <input type="checkbox" class="w-3.5 h-3.5 accent-primary cursor-pointer" :checked="filters.cabin.includes(c)" @change="toggleFilter(filters.cabin, c)" />
    ...
    <div class="flex gap-5">
      ...
      <div class="flex-1 min-w-0 flex flex-col gap-3 md:gap-4">
        ...
        <template v-if="sortedOthers.length">
          ...
          <Pagination
            v-if="sortedOthers.length > itemsPerPage"
            :total-items="sortedOthers.length"
