...
const { passengerCount, isB2B, contactEmail, priceBreakdown } = storeToRefs(flightStore)

const { format, currentConfig } = useCurrency()
const { initPayment } = useFlightBooking()
const toast = useToast()

const gateway = ref<'paystack' | 'flutterwave'>(
  ...
  <div class="space-y-5">
    ...
    <div class="bg-gradient-to-r from-slate-800 to-slate-700 rounded-2xl px-5 py-5 flex items-center justify-between">
      <div>
        ...
        <p class="text-2xl font-bold text-white">{{ format(priceBreakdown.total) }}</p>
        <p class="text-xs text-slate-400 mt-0.5">
          incl. 7.5% VAT ·
          {{ passengerCount }} passenger{{ passengerCount > 1 ? 's' : '' }}
          <span v-if="isB2B" class="text-green-400 ml-1">· B2B discount applied</span>
        </p>
      </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
      ...
      <div class="p-4 grid grid-cols-1 sm:grid-cols-2 gap-3">
        <button
          v-if="currentConfig.code === 'NGN'"
          class="relative flex flex-col items-center justify-center gap-3 p-5 rounded-xl border-2 transition-all cursor-pointer text-left"
      ...
      <div class="px-5 pb-5">
        <div class="flex items-start gap-3 bg-slate-50 border border-slate-200 rounded-xl px-4 py-3.5">
          ...
          <div class="text-xs text-slate-500 space-y-1">
            <p>You'll be taken to <strong class="text-slate-700">{{ gateway === 'paystack' ? 'Paystack' : 'Flutterwave' }}</strong> to complete payment.</p>
            <p>After payment, you'll be redirected back here automatically.</p>
            <p>Your ticket and invoice will be emailed to <span class="font-mono font-medium text-slate-600">{{ contactEmail || 'your email' }}</span>.</p>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="flex gap-3">
      ...
      >
        ...
        <template v-else>
          Pay {{ format(priceBreakdown.total) }} with {{ gateway === 'paystack' ? 'Paystack' : 'Flutterwave' }}
    ...
    <p class="text-center text-xs text-slate-400">
      Payments are processed by {{ gateway === 'paystack' ? 'Paystack' : 'Flutterwave' }} · We never store your card details
    </p>
  </div>
</template>
