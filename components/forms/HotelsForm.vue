...
import { HotelCountryCodeResponse, HotelCountryResponse } from '../../types/hotel';
...
watch(() => form.value.country, async (code) => {
  ...
  try {
    ...
    const res = await hotelService.getHotelCountryCode(code)
    cities.value = res
  } catch (e) {
    console.log(e)
  } finally {
...
const prefillFromQuery = async() => {
  ...
  if (form.value.country) {
    try {
      const res = await hotelService.getHotelCountryCode(form.value.country)
      cities.value = res
    } catch (e) {
      console.log(e)
    }
...
onMounted(async () => {
  try {
    ...
    const res = await hotelService.getHotelCountry()
    countries.value = res
  } catch (e) {
    console.log(e)
  } finally {
  ...
  <div class="grid grid-cols-1 lg:grid-cols-5 gap-4">
    <div>
      ...
      >
        ...
        <option v-for="n in countries" :key="n.code" :value="n.code">
          {{ n.name }}
        </option>
      </select>
    ...
    <div>
      ...
      >
        <option value="">
          {{ loadingCountries ? 'Loading nationalities...' : 'Select Nationality' }}
        </option>
    ...
    <div>
      ...
      <select id="city" v-model="form.city" :disabled="!form.country || loadingCities"
        :class="[
          'input',
