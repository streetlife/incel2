  ...
  <div class="space-y-6">
    <div class="bg-gradient-to-br from-green-500 to-emerald-600 rounded-2xl px-6 py-8 text-center shadow-xl shadow-green-200">
      ...
      <div class="inline-block bg-white/20 rounded-xl px-5 py-2">
        ...
        <p class="text-2xl font-bold text-white font-mono tracking-widest">{{ store.bookingReference }}</p>
      </div>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
      <div class="px-5 py-4 bg-slate-800 flex items-center justify-between">
        ...
        <span class="text-slate-400 text-xs font-mono">{{ store.invoiceNumber }}</span>
      </div>
      ...
      <div class="relative h-32 bg-slate-200 overflow-hidden">
        ...
        <div class="absolute bottom-3 left-5 right-5">
          <div class="flex gap-0.5 mb-1">
            <span
              v-for="n in (store.hotel?.rating ?? 0)" :key="`s${n}`"
              class="text-amber-400 text-xs"
            >★</span>
          </div>
          <p class="text-white font-bold text-base leading-snug">{{ store.hotel?.hotel_name }}</p>
          <p class="text-white/70 text-xs">{{ store.hotel?.hotel_address || store.searchParams.city }}</p>
        </div>
      </div>
      ...
      <div class="px-5 py-4 border-b border-slate-100">
        <div class="grid grid-cols-3 gap-4 text-center">
          <div>
            ...
            <p class="text-sm font-bold text-slate-900">{{ fmtDate(store.searchParams.checkInStart) }}</p>
          </div>
          <div class="flex flex-col items-center justify-center">
            <p class="text-xs text-slate-400 mb-1">Duration</p>
            <div class="flex items-center gap-1">
              <div class="h-px w-5 bg-slate-300" />
              <p class="text-xs font-bold text-slate-600 whitespace-nowrap">{{ store.nights }}n</p>
              <div class="h-px w-5 bg-slate-300" />
            </div>
          </div>
          <div>
            <p class="text-xs text-slate-400 mb-1">Check-out</p>
            <p class="text-sm font-bold text-slate-900">{{ fmtDate(store.searchParams.checkInEnd) }}</p>
          </div>
        </div>
      </div>

      <div class="px-5 py-4 border-b border-slate-100 space-y-2.5">
        <p class="text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-2">Room Details</p>
        <div class="flex justify-between text-sm">
          <span class="text-slate-500">Room type</span>
          <span class="font-medium text-slate-800">{{ store.selectedRoom?.roomName }}</span>
        </div>
        <div class="flex justify-between text-sm">
          <span class="text-slate-500">Board</span>
          <span class="font-medium text-slate-800">{{ store.selectedRoom?.boardType }}</span>
        </div>
        <div class="flex justify-between text-sm">
          <span class="text-slate-500">Rooms</span>
          <span class="font-medium text-slate-800">{{ store.searchParams.totalRooms }}</span>
        </div>
        <div class="flex justify-between text-sm">
      ...
      <div class="px-5 py-4 border-b border-slate-100">
        <p class="text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-3">Guests</p>
      ...
      <div class="flex items-center px-5 relative">
        <div class="w-5 h-5 rounded-full bg-slate-100 -ml-9 border border-slate-200" />
        <div class="flex-1 border-t-2 border-dashed border-slate-200 mx-2" />
        <div class="w-5 h-5 rounded-full bg-slate-100 -mr-9 border border-slate-200" />
      </div>

      <div class="px-5 py-4 bg-slate-50 flex items-center justify-between">
        <div>
          <p class="text-xs text-slate-400">Total Paid</p>
          <p class="text-xl font-bold text-slate-900">{{ format(store.priceBreakdown.total, currency) }}</p>
          <p class="text-[10px] text-slate-400 mt-0.5">incl. VAT · {{ currency }}</p>
        </div>
