  ...
  >
    ...
    <div class="px-5 py-4 border-b border-slate-100 space-y-2.5">
      ...
      <div class="flex justify-between text-sm">
        ...
        <span class="font-medium text-slate-800"
          >{{ store.nights }} night{{ store.nights !== 1 ? "s" : "" }}</span
        >
      </div>
      <div class="flex justify-between text-sm">
    ...
    <div
      v-if="store.selectedRoom"
      class="px-5 py-4 border-b border-slate-100 space-y-2"
    >
      <p
        class="text-xs font-bold uppercase tracking-widest text-slate-400 mb-3"
      >
    ...
    <div v-if="store.selectedRoom" class="px-5 py-4 space-y-2.5">
      ...
      <div
        class="border-t border-dashed border-slate-200 pt-3 flex justify-between items-baseline"
      >
    ...
    <div
      class="mx-5 mb-5 flex items-start gap-2 bg-blue-50 border border-blue-200 rounded-xl px-3 py-2.5"
    >
      <svg
      ...
      <p class="text-xs text-primary">
        Invoice & booking voucher will be emailed after payment.
      </p>
    </div>
  </div>
</template>
