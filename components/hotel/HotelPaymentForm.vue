...
const customerName = computed(() =>
  [primaryGuest.value?.title, primaryGuest.value?.firstName, primaryGuest.value?.lastName]
    .filter(Boolean)
    .join(' '),
)
...
async function handlePay() {
  ...
  try {
    ...
    if (!paymentLink) throw new Error('No payment link returned. Please try again.')

    globalThis.location.href = paymentLink
  } catch (e: any) {
    error.value   = e?.message ?? 'Could not connect to payment gateway. Please try again.'
    loading.value = false
  }
}
</script>
  ...
  <div class="space-y-5">
    <div class="bg-gradient-to-r from-slate-800 to-slate-700 rounded-2xl px-5 py-5 flex items-center justify-between">
      <div>
        ...
        <p class="text-2xl font-bold text-white">{{ format(store.priceBreakdown.total, currency) }}</p>
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
      ...
      <div class="p-5">
        ...
        >
          ...
          >
            <span class="text-white font-black text-lg">{{ meta.logoLetter }}</span>
          </div>
      ...
      <div class="px-5 pb-5">
        <div class="flex items-start gap-3 bg-slate-50 border border-slate-200 rounded-xl px-4 py-3.5">
          ...
          <p class="text-xs text-slate-500">
            ...
            to complete payment securely, then redirected back here.
            Your voucher will be emailed to
            <span class="font-mono font-medium text-slate-600">{{ store.contactEmail }}</span>.
          </p>
        </div>
      </div>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
      ...
      <div class="px-5 py-4 space-y-2.5">
        ...
        <div class="flex justify-between items-center pt-3 border-t border-dashed border-slate-200">
          ...
          <span class="text-lg font-bold text-primary">{{ format(store.priceBreakdown.total, currency) }}</span>
        </div>
      </div>

      <div class="px-5 pb-4 border-t border-slate-100 pt-3.5">
        <p class="text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-2">Billing To</p>
    ...
    <div class="flex gap-3">
      ...
      >
        ...
        <template v-else>
          Pay {{ format(store.priceBreakdown.total, currency) }} via {{ meta.label }}
