...
const { state, priceBreakdown, fmtNgn } = useHotelBookingStore()
...
async function handlePay() {
  ...
  try {
    ...
    // Backend calls Paystack/Flutterwave, then on webhook → calls Rezlive booking API:
    //   POST https://api.rezlive.com/hotel/book
    //   { rezliveRoomId, checkIn, checkOut, guests, contactEmail, paymentRef, nationality }
    // Then emails invoice + voucher PDF to contactEmail.
    const res = await fetch('/api/payments/initialize', {
      ...
      body: JSON.stringify({
        ...
        metadata: {
          ...
          rezliveRoomId: state.selectedRoom?.rezliveRoomId,
          checkIn:       state.searchParams.checkInStart,
          checkOut:      state.searchParams.checkInEnd,
          nationality:   state.searchParams.nationality,
          totalRooms:    state.searchParams.totalRooms,
          guests: state.guests.map(g => ({ title: g.title, firstName: g.firstName, lastName: g.lastName })),
        },
        callbackUrl: `${globalThis.location.origin}/travel/hotels/booking/confirm`,
      }),
    })
  ...
  <div class="space-y-5">
    ...
    <div class="bg-gradient-to-r from-slate-800 to-slate-700 rounded-2xl px-5 py-5 flex items-center justify-between">
      <div>
        ...
        <p class="text-xs text-slate-400 mt-0.5">incl. 7.5% VAT · {{ state.searchParams.totalRooms }} room{{ state.searchParams.totalRooms !== 1 ? 's' : '' }}</p>
      </div>
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
      ...
      <div class="px-5 pb-5">
        <div class="flex items-start gap-3 bg-slate-50 border border-slate-200 rounded-xl px-4 py-3.5">
          ...
          <p class="text-xs text-slate-500">You'll be taken to <strong class="text-slate-700">{{ gateway === 'paystack' ? 'Paystack' : gateway === 'flutterwave' ? 'Flutterwave' : 'your chosen gateway' }}</strong> to complete payment, then redirected back here. Your voucher will be emailed to <span class="font-mono font-medium text-slate-600">{{ state.contactEmail }}</span>.</p>
        </div>
      </div>
    </div>
