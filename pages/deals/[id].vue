...
function isDealLive() {
  ...
  const now = Date.now();
  const start = deal.value.start_date
    ? new Date(deal.value.start_date).getTime()
    : 0;
  const end = deal.value.end_date
    ? new Date(deal.value.end_date).getTime()
    : Infinity;
...
onMounted(async () => {
  await Promise.all([fetchRates(), fetchDeal(), hotDealsStore.fetchDeals()]);
  tick();
...
useHead({
  title: computed(() =>
    deal.value ? `${deal.value.title} — Incel Tourism` : "Deal — Incel Tourism",
  ),
  link: [
    {
      rel: "stylesheet",
  ...
  <div class="min-h-screen bg-[#040d1a]">
    <div v-if="loading" class="flex items-center justify-center min-h-screen">
      <div class="text-center">
        <div
          class="w-12 h-12 border-4 border-yellow-400/30 border-t-yellow-400 rounded-full animate-spin mx-auto mb-4"
        />
    ...
    <div
      v-else-if="error"
      class="flex flex-col items-center justify-center min-h-screen gap-6 px-4 text-center"
    >
      <div class="text-6xl mb-2">⚠️</div>
      <h1 class="text-3xl font-black text-white">Failed to load deal</h1>
      <p class="text-slate-400 max-w-sm">
        Something went wrong fetching this deal. Please try again.
      </p>
      <button
        @click="fetchDeal"
    ...
    <template v-else>
      <section class="relative overflow-hidden">
          ...
          style="
            background:
              radial-gradient(
                ...
                rgba(1, 104, 167, 0.4) 0%,
                transparent 65%
              ),
              radial-gradient(
                ellipse 60% 60% at 80% 70%,
                rgba(1, 66, 122, 0.35) 0%,
                transparent 60%
              ),
              #040d1a;
          "
        ...
        <div class="relative z-10 container mx-auto sm:px-24 px-4 pt-36 pb-16">
          ...
          <div
            v-if="isDealLive()"
            class="inline-flex items-center gap-2 mb-4 px-4 py-1.5 rounded-full text-xs font-bold tracking-widest uppercase text-green-400 border border-green-400/30 bg-green-400/10"
          >
          ...
          <div
            ...
            class="inline-flex items-center gap-2 text-sm text-slate-400 bg-white/[0.06] border border-white/[0.08] px-4 py-2 rounded-full"
          >
            <svg
      ...
      <section class="container mx-auto sm:px-24 px-4 py-12">
        <div class="grid lg:grid-cols-3 gap-8">
          <div class="lg:col-span-2 flex flex-col gap-6">
            <div
              class="rounded-2xl bg-white/[0.04] border border-white/[0.08] p-6 sm:p-8"
            >
              <h2
            ...
            >
              ...
              <div class="grid sm:grid-cols-2 gap-3">
                ...
                >
                  <span
                    class="w-6 h-6 rounded-full bg-yellow-400/20 flex items-center justify-center shrink-0"
                  >
                    <svg
          ...
          <div class="lg:col-span-1">
            <div
              class="sticky top-28 rounded-2xl bg-white/[0.06] border border-white/[0.10] p-6 flex flex-col gap-5"
            >
              <div
                class="inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-bold tracking-widest uppercase text-yellow-400 border border-yellow-400/30 bg-yellow-400/10 self-start"
              >
                <span
              ...
              <div
                v-if="deal.end_date"
                class="rounded-xl bg-[#0168a7]/20 border border-[#0168a7]/30 p-4"
              >
                <p
                  class="text-xs text-slate-400 mb-2 flex items-center gap-1.5"
                >
                  <svg
                    class="w-3.5 h-3.5 text-yellow-400"
                    fill="none"
                  ...
                  <span v-if="dealExpired" class="text-red-400"
                    >This deal has expired</span
                  >
                  <span v-else
                    >Offer expires {{ formatDealDate(deal.end_date) }}</span
                  >
                </p>
                <div v-if="!dealExpired" class="flex items-center gap-2">
                  <template v-for="(unit, i) in countdown" :key="i">
                    <div
                      class="text-center flex-1 bg-white/[0.05] rounded-lg py-2"
                    >
                      <div
                        class="text-yellow-400 font-black text-xl leading-none"
                        style="
                          font-family:
                            Bebas Neue,
                            sans-serif;
                      ...
                      <div
                        class="text-[0.55rem] text-slate-500 tracking-widest mt-0.5"
                      >
                        {{ unit.label }}
                      </div>
                    </div>
      ...
      <section
        v-if="otherDeals.length"
        class="container mx-auto sm:px-24 px-4 pb-24"
      >
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          >
            <div
              class="h-12 bg-gradient-to-br from-[#0168a7]/50 to-[#051020]"
            ></div>
            <div class="p-4">
              <h3
                class="text-white font-bold text-lg group-hover:text-yellow-300 transition-colors mb-1 leading-tight"
              >
                {{ other.title }}
              </h3>
              <p class="text-slate-400 text-xs mb-3 line-clamp-2">
                {{ other.deal_includes }}
              </p>
              <div class="flex items-end justify-between">
                <div
                  class="text-xl font-black text-yellow-400"
                  style="
                    font-family:
                      Bebas Neue,
                      sans-serif;
