...
const openDeal  = (deal: HotDeal) => { selectedDeal.value = deal }
const closeDeal = () => { selectedDeal.value = null }
...
<template>
  <div class="min-h-screen bg-[#040d1a] font-sans">
    <section class="relative min-h-[480px] flex flex-col justify-end overflow-hidden">
      ...
      <div class="relative z-10 container mx-auto sm:px-24 px-4 pt-36 pb-24 flex flex-col items-start gap-6">
        ...
        <p class="text-lg text-slate-300 max-w-lg leading-relaxed">
          Exclusive packages curated for you — all-inclusive escapes at prices that won't last long.
        </p>
      </div>
    ...
    <div class="container mx-auto sm:px-24 px-4 -mt-2 mb-10">
      <div class="rounded-2xl px-6 py-4 flex flex-col sm:flex-row items-center justify-between gap-4 border border-[#0168a7]/40 bg-[#0168a7]/20">
        <div class="flex items-center gap-3">
          ...
          <span class="text-white text-sm font-medium">Soonest deal expires in:</span>
        </div>
        <div class="flex items-center gap-3">
          <template v-for="(unit, i) in countdown" :key="i">
            <div class="text-center">
              <div class="text-yellow-400 min-w-[2.5rem] text-center leading-none" style="font-family: 'Bebas Neue', sans-serif; font-size: 2rem;">{{ unit.value }}</div>
    ...
    <section class="container mx-auto sm:px-24 px-4 pb-24">
      <div v-if="hotDealsStore.loading" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
        <div v-for="i in 4" :key="i" class="rounded-2xl overflow-hidden bg-white/[0.04] border border-white/[0.08] animate-pulse">
      ...
      <div v-else-if="hotDealsStore.error" class="text-center py-16">
        ...
        <button @click="hotDealsStore.refresh()" class="px-6 py-2.5 rounded-full font-bold text-sm bg-yellow-400 text-[#040d1a] hover:bg-yellow-300 transition-colors">
          Retry
        </button>
      </div>
      ...
      <template v-else-if="hotDealsStore.activeDeals.length">
        ...
        >
          <div class="absolute inset-0 pointer-events-none"
            style="background: radial-gradient(ellipse 70% 80% at 80% 50%, rgba(1,104,167,0.25) 0%, transparent 70%)"
          />

          <div class="relative z-10 p-8 sm:p-12 flex flex-wrap items-end justify-between gap-6">
            <div class="flex-1 min-w-0">
              <div v-if="isDealLive(featuredDeal)" class="inline-flex items-center gap-1.5 mb-3 text-[0.7rem] font-bold tracking-widest uppercase px-3 py-1 rounded-full bg-green-500/20 text-green-400 border border-green-500/30">
                <span class="w-1.5 h-1.5 rounded-full bg-green-400 animate-pulse"></span>
                Live Now
              </div>

              <h2 class="text-3xl sm:text-4xl font-black text-white mb-2 group-hover:text-yellow-300 transition-colors leading-tight">
                {{ featuredDeal.title }}
              </h2>
              <p class="text-slate-300 text-base max-w-xl leading-relaxed">{{ featuredDeal.deal_includes }}</p>

              <div v-if="featuredDeal.start_date" class="flex items-center gap-1.5 mt-4 text-xs text-slate-400">
                <svg class="w-3.5 h-3.5 text-yellow-400 shrink-0" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/>
                </svg>
                {{ formatDealDate(featuredDeal.start_date) }} – {{ formatDealDate(featuredDeal.end_date) }}
              </div>

              <div v-if="featuredDeal.other_info?.length" class="flex flex-wrap gap-2 mt-4">
                <span
            ...
            <div class="text-right shrink-0">
              <div class="text-4xl font-black text-yellow-400" style="font-family: 'Bebas Neue', sans-serif;">
                {{ format(featuredDeal.price) }}
              </div>
              <button class="mt-4 px-8 py-3 rounded-full font-extrabold text-sm bg-yellow-400 text-[#040d1a] shadow-[0_4px_20px_rgba(255,193,7,0.35)] hover:bg-yellow-300 hover:-translate-y-0.5 transition-all duration-200">
                View Deal →
              </button>
            </div>
          </div>
        ...
        <div v-if="regularDeals.length" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          <div
            ...
            @click="openDeal(deal)"
          >
            <div class="relative h-16 bg-gradient-to-br from-[#0168a7]/60 to-[#01427a]/60 flex items-center px-5 gap-3 shrink-0">
              <div class="w-8 h-8 rounded-full bg-yellow-400/20 flex items-center justify-center shrink-0">
              ...
              <div v-if="isDealLive(deal)" class="ml-auto flex items-center gap-1 text-[0.65rem] font-bold text-green-400">
                <span class="w-1.5 h-1.5 rounded-full bg-green-400 animate-pulse"></span> LIVE
              </div>
            </div>

            <div class="p-5 flex flex-col flex-grow">
              <h3 class="text-white font-bold text-xl mb-2 group-hover:text-yellow-300 transition-colors leading-tight">{{ deal.title }}</h3>
              <p class="text-slate-400 text-sm mb-3 leading-relaxed line-clamp-2">{{ deal.deal_includes }}</p>

              <div v-if="deal.other_info?.length" class="flex flex-wrap gap-1.5 mb-4">
                <span
              ...
              <div v-if="deal.start_date" class="flex items-center gap-1.5 text-xs text-slate-500 mb-4">
                <svg class="w-3 h-3 text-yellow-400/60 shrink-0" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/>
                </svg>
                {{ formatDealDate(deal.start_date) }} – {{ formatDealDate(deal.end_date) }}
              </div>

              <div class="mt-auto flex items-end justify-between pt-4 border-t border-white/[0.06]">
                <div class="text-2xl font-black text-yellow-400" style="font-family: 'Bebas Neue', sans-serif;">
                  {{ format(deal.price) }}
                </div>
                <button class="px-5 py-2.5 rounded-full font-extrabold text-sm bg-yellow-400 text-[#040d1a] shadow-[0_4px_20px_rgba(255,193,7,0.3)] hover:bg-yellow-300 hover:-translate-y-0.5 transition-all duration-200">
                  Book →
                </button>
              </div>
            </div>
      ...
      <div v-else-if="!hotDealsStore.loading" class="text-center py-24">
        ...
        <p class="text-slate-400 text-lg">No active deals right now — check back soon!</p>
      </div>
    </section>
    ...
    >
      <div v-if="selectedDeal" class="fixed inset-0 z-[200] flex items-center justify-center p-4">
        <div class="absolute inset-0 bg-[#040d1a]/85 backdrop-blur-md" @click="closeDeal" />

        <div class="relative z-10 w-full max-w-2xl rounded-3xl overflow-hidden bg-[#0a1628] border border-white/10 shadow-2xl max-h-[90vh] overflow-y-auto">
          <div class="relative bg-gradient-to-br from-[#0168a7]/60 to-[#051020] px-8 py-10">
            <div class="absolute inset-0 pointer-events-none"
              style="background: radial-gradient(ellipse 80% 80% at 100% 0%, rgba(1,104,167,0.3) 0%, transparent 70%)"
            />
            <button
              @click="closeDeal"
            ...
            <div v-if="isDealLive(selectedDeal)" class="inline-flex items-center gap-1.5 mb-3 text-[0.7rem] font-bold tracking-widest uppercase px-3 py-1 rounded-full bg-green-500/20 text-green-400 border border-green-500/30">
              <span class="w-1.5 h-1.5 rounded-full bg-green-400 animate-pulse"></span> Live Now
            </div>
            <h2 class="text-3xl font-black text-white relative z-10">{{ selectedDeal.title }}</h2>
            <div v-if="selectedDeal.start_date" class="flex items-center gap-1.5 mt-2 text-xs text-slate-400 relative z-10">
              <svg class="w-3.5 h-3.5 text-yellow-400 shrink-0" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/>
              </svg>
              {{ formatDealDate(selectedDeal.start_date) }} – {{ formatDealDate(selectedDeal.end_date) }}
            </div>
          </div>

          <div class="p-8">
            <p class="text-slate-300 mb-6 leading-relaxed">{{ selectedDeal.deal_includes }}</p>

            <div v-if="selectedDeal.other_info?.length" class="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-6">
              <div
            ...
            <div class="flex items-end justify-between border-t border-white/[0.08] pt-6">
              <div>
