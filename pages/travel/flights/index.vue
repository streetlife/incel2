...
// ── Book Now: store offer in sessionStorage, navigate to booking flow ─────────
const handleBooking = (id: string) => {
  const flight = flightResults.value.find(f => f.id === id)
  if (!flight) return

  sessionStorage.setItem('selectedFlight', JSON.stringify({
    offer: flight,
    passengerCount: Number(route.query.passengers) || 1,
  }))

  router.push({
    path: '/travel/flights/booking',
    query: { offerId: id, step: '1' },
  })
}
    ...
    >
      ...
      <div class="relative z-10 max-w-7xl mx-auto">
        <div class="text-center reveal transition-all duration-500" :class="hasSearched ? 'mb-6' : 'mb-12'">
          <h1 class="font-bold mb-4 text-white transition-all duration-500" :class="hasSearched ? 'text-3xl md:text-4xl' : 'text-4xl md:text-5xl'">
            {{ hasSearched ? 'Search Flights' : 'Book Your Flight' }}
          </h1>
          <p v-if="!hasSearched" class="text-xl text-gray-100">Find the best deals on flights to destinations worldwide</p>
        </div>
    ...
    <section v-if="hasSearched" id="search-results" class="py-10 px-6 bg-slate-100">
      <div class="max-w-7xl mx-auto">
        ...
        <div v-else class="flex flex-col items-center gap-3 py-20 text-center">
          ...
          <button class="mt-2 px-8 py-3 bg-gray-900 hover:bg-gray-800 text-white rounded-xl font-semibold transition-colors cursor-pointer border-none" @click="toggleSearchForm">
            Modify Search
          </button>
        </div>
      </div>
