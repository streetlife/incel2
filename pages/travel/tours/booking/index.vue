...
import TourPackageSelect from '../../../../components/tour/TourPackageSelect.vue'
import TourLeadForm from '../../../../components/tour/TourLeadForm.vue'
import TourBookingReview from '../../../../components/tour/TourBookingReview.vue'
import TourPaymentForm from '../../../../components/tour/TourPaymentForm.vue'
...
onMounted(() => {
  const raw = sessionStorage.getItem('selectedTour')
  if (raw) {
    try {
      const { tour, searchParams } = JSON.parse(raw)
      setTour(tour, searchParams)
    } catch {}
  }
  if (!state.tour && step.value < 5) {
    router.replace('/travel/tours')
  }
})
...
function goBack() {
  if (step.value === 1) { router.push('/travel/tours'); return }
  ...
  <div class="min-h-screen bg-slate-100 pt-36 pb-16 sm:px-24 px-4">
    <div class="max-w-6xl mx-auto">
      ...
      <div class="mb-8">
        ...
        <p class="text-slate-500 text-sm mt-1">{{ state.tour?.name }} · {{ state.searchParams.city }}</p>
      </div>
        ...
        <div class="flex items-center gap-0">
          <template v-for="(s, i) in STEPS.slice(0, 4)" :key="s.id">
            <div class="flex flex-col items-center gap-1.5">
              ...
              <span class="text-xs font-medium hidden sm:block" :class="step >= s.id ? 'text-slate-700' : 'text-slate-400'">
                {{ s.label }}
              </span>
            </div>
            <div v-if="i < 3"
      ...
      <div :class="showSidebar ? 'grid gap-6 lg:grid-cols-[1fr_320px]' : 'max-w-2xl mx-auto'">
        ...
        <div class="min-w-0">
          ...
          >
            <TourPackageSelect v-if="step === 1" @next="goNext" />
            <TourLeadForm v-else-if="step === 2" @next="goNext" @back="goBack" />
            <TourBookingReview v-else-if="step === 3" @next="goNext" @back="goBack" />
            <TourPaymentForm v-else-if="step === 4" @back="goBack" />
            <TourBookingConfirmation v-else-if="step === 5" />
          </Transition>
        </div>
