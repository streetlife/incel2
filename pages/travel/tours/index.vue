...
import { Map, Search } from 'lucide-vue-next'
...
const selectedTourTypes = ref<string[]>([])  // ← NEW

// ── Derived filter options from results ────────────────────────────────────────
const availableTourTypes = computed(() => {           // ← NEW
  const types = searchResults.value.map((t: any) => t.cityTourType).filter(Boolean)
...
const paginatedResults = computed(() => {
  ...
  return filteredResults.value.slice(start, start + itemsPerPage)  // ← uses filteredResults
})
  ...
  <div class="min-h-screen">
    ...
    <section v-if="hasSearched" id="search-results" class="py-16 px-6 bg-gray-50">
      <div class="max-w-7xl mx-auto">
        ...
        <template v-else-if="searchResults.length > 0">
          ...
          <div class="flex flex-col lg:flex-row gap-8">
            ...
            <aside class="lg:w-64 shrink-0">
              <div class="bg-white rounded-2xl shadow-md p-5 sticky top-6">
                <div class="flex items-center justify-between mb-4">
                  ...
                  <button
                    v-if="selectedTourTypes.length > 0"
                    class="text-xs text-gray-500 hover:text-gray-900 underline underline-offset-2 transition-colors cursor-pointer border-none bg-transparent"
                ...
                <ul class="space-y-2">
                  <li v-for="type in availableTourTypes" :key="type">
                    <label class="flex items-center gap-3 cursor-pointer group">
                      <span
                        class="w-4 h-4 shrink-0 rounded border-2 flex items-center justify-center transition-colors"
                        :class="selectedTourTypes.includes(type)
                          ? 'bg-gray-900 border-gray-900'
                          : 'border-gray-300 group-hover:border-gray-500'"
                      >
                        <svg v-if="selectedTourTypes.includes(type)" class="w-2.5 h-2.5 text-white" viewBox="0 0 10 10" fill="none">
                      ...
                      <input
                        ...
                        @change="toggleTourType(type)"
                      />
                      <span class="text-sm text-gray-700 group-hover:text-gray-900 transition-colors leading-tight">{{ type }}</span>
                      <span class="ml-auto text-xs text-gray-400 tabular-nums">
                        {{ searchResults.filter((t: any) => t.cityTourType === type).length }}
                      </span>
                    </label>
                  </li>
                </ul>
                ...
                <div v-if="selectedTourTypes.length > 0" class="mt-4 pt-4 border-t border-gray-100">
                  <p class="text-xs text-gray-500">
                    Showing <strong class="text-gray-800">{{ filteredResults.length }}</strong>
                    of {{ searchResults.length }} tour{{ searchResults.length !== 1 ? 's' : '' }}
                  </p>
                </div>
              </div>
            </aside>
            ...
            <div class="flex-1 min-w-0">
              ...
              <!-- Tour cards -->
              <div v-if="paginatedResults.length > 0" class="grid gap-6">
                <div
              ...
              <div v-else class="text-center py-20">
                <div class="text-6xl mb-4 flex justify-center"><Search class="text-center" :size="48" /></div>
                <h3 class="text-2xl font-bold text-gray-900 mb-2">No Matches</h3>
                <p class="text-gray-600 mb-6">No tours match the selected types. Try selecting different filters.</p>
                <button
                  class="px-8 py-3 bg-gray-900 text-white rounded-lg font-semibold hover:bg-gray-800 transition-colors cursor-pointer border-none"
                  @click="clearTypeFilters"
                >
                  Clear Filters
                </button>
        ...
        <!-- No results from API -->
        <div v-else-if="!searchLoading && !searchError" class="text-center py-20">
          <div class="text-6xl mb-4 flex justify-center"><Map class="text-center" :size="48" /></div>
