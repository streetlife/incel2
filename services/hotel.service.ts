...
export function useHotelService() {
  ...
  return {
    ...
    async getHotelCountry(): Promise<HotelCountryResponse[]> {
      return $api<HotelCountryResponse[]>("/hotels/countries", {
        method: "GET",
      });
    },

    async getHotelCountryCode(
      code: string,
    ): Promise<HotelCountryCodeResponse[]> {
      return $api<HotelCountryCodeResponse[]>(`/hotels/cities/${code}`, {
        method: "GET",
      });
    },
    ...
    async getHotelById(
      sessionCode: string,
      hotelId: string,
    ): Promise<HotelByIdResponse> {
      const res = await $api<HotelByIdResponse>(
        `/hotels/${sessionCode}/${hotelId}`,
        {
          method: "GET",
        },
      );
    ...
    async flutterwavePayment(
      params: FlutterwavePaymentData,
    ): Promise<FlutterwavePaymentResponse> {
      return $api<FlutterwavePaymentResponse>(
    ...
    async preBooking(params: PreBookingData): Promise<PreBookResponse> {
      const res = await $api<ApiResponse<PreBookResponse>>("/hotels/prebook", {
        method: "POST",
        body: params,
      });
...
function buildPayload(params: HotelSearchParams): Record<string, unknown> {
  if (!params.city) {
    throw new Error(`City "${params.city}" is not found.`);
  }
